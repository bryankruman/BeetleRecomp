...
//
// Status (2026-06-28 night): boots. RT64 (D3D12) initializes, the SDL window opens, the ROM is
// recognized, and the recompiled boot runs through libultra init -> the App thread -> the main
// function -> BAR's uv module loader. It currently aborts in get_function() when uvLoadModuleCode
// jumps into a relocatable module that librecomp's func_map doesn't know about (the "~130
// relocatable modules" overlay bridge — the next milestone). See docs/STATUS.md.
...
// Set true by the factory above once RT64 has initialized; gates the deferred start_game() below.
extern std::atomic<bool> g_bar_renderer_ready;
...
int main(int argc, char** argv) {
    ...
    std::thread([game_id]() {
        while (!g_bar_renderer_ready.load()) {
