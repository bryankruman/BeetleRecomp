...
> Note on current state: audio is still stubbed, input is partly a bring-up hack, and a forced
> race crashes ~30 s in (see [STATUS.md](STATUS.md) and project memory). Steps 1-2 are safe to
> do in parallel with that; steps 3-5 are larger and assume a stabler base.
